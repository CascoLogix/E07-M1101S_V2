...
void main(void)
{
	...
	GPIO_PORT1_registerCallback(SW2_PIN_NUM, &SW2_callback, LPM0_bits);		// Should toggle the LED when the button is pressed.
...
void SW2_callback (void)
{
	...
	SW2_PxIE &= ~SW2_PIN;					// Disable SW2 interrupt; will re-enable after TX complete
...
void periodicTX (void)
{
	...
	LED2_ON;
	currentTime++;
	FLAGS |= SW2FLAG;								// Will cause a transmit
